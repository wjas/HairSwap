...
- 保存结果图片
- 查看生成历史记录
- 支持多模型切换（4.5/5.0 lite）

**技术特点**：
- 使用火山引擎 doubao-seedream-5-0-260128 模型（默认）
- 支持 doubao-seedream-4-5 模型切换
- 多图融合技术，保持脸部、背景不变
- 无需用户登录，即开即用
- 文件系统存储历史记录（永久保存）
- 保护用户隐私，不存储上传照片
...
**当前版本**：v1.6.0（2026-03-10）

---

## 🔧 重要问题修复

### 图片格式优化与查看原图功能修复（2026-03-10 修复）

**问题描述**：
- 🔴 火山引擎返回的 JPEG 图片被强制转成 PNG，体积变大（从约 600K 变成 4MB）
- 🔴 历史记录详情页查看原图时，显示的是最后一次上传的照片，而不是对应历史记录的原图
- 🔴 用户上传的非 PNG 格式照片被强制转成 PNG 保存

**根本原因**：
1. 状态管理不当：`photoBase64` 状态没有在进入历史记录详情页时清空
2. 强制格式转换：使用 sharp 库强制将图片转成 PNG，导致体积增加
3. 没有检测原始格式：用户上传的照片和火山引擎返回的图片都被强制转成 PNG

**解决方案**：
1. ✅ 完善状态管理：进入历史记录详情页时清空 `photoBase64`，上传新照片时清空 `originalImageUrl`
2. ✅ 保持原始格式：移除 sharp 重新编码，直接保存火山引擎原始图片（JPEG 格式，体积约 600K）
3. ✅ 检测并保持用户上传照片原始格式：自动检测格式，保存为正确扩展名（original.jpg 或 original.png）
4. ✅ 历史记录接口支持查找不同格式的文件

**详细记录**：详见 [`iteration-log.html`](iteration-log.html)

### 5G 网络无法生图问题（2026-03-10 修复）

**问题描述**：
- 🔴 手机使用 5G 网络访问时，生成图片提示"load failed"
- 🟢 WiFi 环境下一切正常

**根本原因**：
1. 前端代码硬编码了局域网 IP `192.168.2.60`
2. 服务器缺少 `.env` 配置文件（API Key 未加载）

**解决方案**：
1. ✅ 修改前端代码使用动态获取的服务器地址
2. ✅ 上传 `.env` 文件到服务器
3. ✅ 重启后端服务
...
项目包含移动端调试工具，无需连接 Mac 即可查看控制台日志。

**启用方法**：
1. 取消 `h5/index.html` 中调试按钮的注释
2. 取消 `h5/js/app.js` 中 `this.showVersion()` 的注释
3. 上传到服务器

**功能特性**：
- 🔴 红色 DEBUG 按钮：点击显示控制台日志
- 🔵 蓝色版本号按钮：验证缓存是否刷新

**使用说明**：详见 [`DEBUG_TOOL.md`](DEBUG_TOOL.md)
...
│   │   └── [记录 ID]/        # 每条记录独立文件夹
│   │       ├── metadata.json # 元数据
│   │       ├── original.{jpg/png} # 原图（保持原始格式）
│   │       ├── result.{jpg/png}   # 生成图（保持火山引擎原始格式）
